...
public class BoardTestSuite {
    ...
    public void testAddTaskListAverageWorkingOnTask() {
        ...
        long tasksToDo = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .count();

        long workingDays = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
        ...
        double average = workingDays / tasksToDo;


        //Then
        Assert.assertEquals(3, tasksToDo);
        Assert.assertEquals(30, workingDays);
